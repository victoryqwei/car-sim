class Box {
  ...
  applyPhysics() {
    ...
    this.wheels.front.slipAngle =
      Math.atan2(this.localVelocity.y + yawSpeedFront, Math.abs(this.localVelocity.x)) -
      Math.sign(this.localVelocity.x) * this.steeringAngle;
    ...
    var frictionForceFront =
      Math.clamp(-this.cornerStiffnessFront * this.wheels.front.slipAngle, -tireGripFront, tireGripFront) * axleWeightFront;
    var frictionForceRear =
      Math.clamp(-this.cornerStiffnessRear * this.wheels.back.slipAngle, -tireGripRear, tireGripRear) * axleWeightRear;

    // Frictional forces
    let dragForce = new Vector(
    ...
    for (var j = 0; j < 4; j++) {
      ...
      for (var i = j; i < particles.length; i += 4) {
        ...
        var inArena =
          lastP.x > 0 &&
          lastP.y > 0 &&
          lastP.x < canvas.width &&
          lastP.y < canvas.height &&
          p.x > 0 &&
          p.y > 0 &&
          p.x < canvas.width &&
          p.y < canvas.height;
