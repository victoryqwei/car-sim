...
let scale = 25; // (scale) px = 1 meter
let useMouse = false;
let smoothSteer = true;

// Create a new car
let car = new Car();
let car2 = new Car(1000, canvas.height / 2);

let boxes = [
  new Box(canvas.width / 2, canvas.height / 2) /*, new Box(canvas.height/2, canvas.height/4), new Box(canvas.height/2, canvas.height*3/4)*/,
];
/*for (let i = 0; i < 5; i++){
...
function checkCollision() {
  ...
  let entities = boxes.concat([car, car2]);
...
function drawControls() {
  ...
  if (useMouse) {
    ...
    drawText(
      "Idle",
      ctrl.padding,
      ctrl.idlePosition + (ctrl.brakePosition - ctrl.idlePosition) / 2,
      "15px Arial",
      "black",
      "left",
      "middle"
    ...
    drawText(
      "W / Up - accelerate, S / Down - brake, A / D / Left / Right - steer, Space - emergency brake",
      canvas.width / 2,
      canvas.height - 25,
      "",
      "black",
...
function drawMenu() {
  ...
  if (car.speed > 20 || car2.speed > 20) showMenu = false;
}

let stop = false;
let unstopToggle = false;
let unstopDelay = Date.now();
  ...
  if (map[82]) {
    ...
    boxes = [
      new Box(
        canvas.width / 2,
        canvas.height / 2
      ) /*, new Box(canvas.height/2, canvas.height/4), new Box(canvas.height/2, canvas.height*3/4)*/,
    ];
    points = [];
  }
}
...
function draw() {
  ...
  if (showInstructions) drawControls();
}
...
function addStat(name, value, decimalPlace, averageArray, maxCount) {
  ...
  if (averageArray) {
    ...
    if (averageArray.length > (maxCount || 30)) averageArray.shift();
